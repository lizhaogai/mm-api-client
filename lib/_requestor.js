...
const log = debug('mm-api-client:request');
...
class Requestor {
   ...
   * @param {String} [opts.apiBase=http://localhost/rest/v0] - the base API URL
   */
  constructor(opts = {}) {
    this.__auth = null;
    this.configure(opts);
  ...
  configure(opts = {}) {
    this.__apiBase = opts.base || opts.baseUrl || this.__apiBase || 'http://localhost/rest/v0';
    if (opts.auth) {
      this.auth(opts.auth);
    }
  }
